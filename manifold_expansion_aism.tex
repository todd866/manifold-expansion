...
\end{equation}
By upper semicontinuity of the differential's rank (the sets $\{\beta : \mathrm{rank}\, dF_\kappa(\beta) \geq r\}$ are open for smooth maps), the set where rank equals the supremum is open and dense. We call $r(\kappa)$ the ``generic rank'' when this open set is nonempty; in applications we restrict to parameter regions where the supremum is attained.
...
\caption{\textbf{Coupled OU: correlation emerges under coupling.} (A) The correlation $\rho = \kappa/(\gamma + 2\kappa)$ increases monotonically from zero, asymptoting to 0.5. (B) Geometric interpretation: at $\kappa = 0$, the accessible family lies on the independence submanifold ($\rho = 0$); coupling moves the trajectory into the transverse direction.}
\label{fig:ou}
\end{figure}
...
\begin{itemize}
\item (S1): For $K < K_c$, $F_K(\mathcal{B})$ is contained in the $S^1$-fixed point set (the incoherent state).
\item (S2): For $K > K_c$, the synchronized stationary solutions (gauge-fixed to $\Psi = 0$) break $S^1$ symmetry in the sense that $r > 0$.
\item (S3): The $S^1$-orbit of synchronized solutions is 1-dimensional.
\item (S4) \textbf{Kernel-to-orbit activation}: Consider a 1-parameter family of frequency distributions $\nu_s$ (e.g., shifting the mean $\omega_0 \mapsto \omega_0 + s$). For $K < K_c$, varying $s$ leaves the incoherent distribution unchanged, so $\partial F_K/\partial s = 0$. For $K > K_c$, the synchronized solution's mean phase responds to frequency shifts: $\partial \Psi/\partial s \neq 0$, activating the orbit direction.
\end{itemize}

The theorem predicts: $r(K) \geq r(0) + 1$ for $K > K_c$. The ``$+1$'' comes from the orbit direction becoming identifiable.
