...
\noindent\textit{To be precise:} We study changes in the \textbf{dimension of the reachable model family} under interaction, not the rate of symbol transmission through a fixed channel.
...
\begin{enumerate}
    ...
    \item \textbf{Section~\ref{sec:consequences}}: Consequences for complexity growth, including the eigenvalue emergence corollary (Corollary~\ref{cor:eigenvalue})
...
\end{enumerate}
Geometrically, (C2) says $F_\kappa$ becomes transverse to $\mathcal{M}_0$ at $F_\kappa(\beta_0)$: the image acquires a component in the normal bundle $N\mathcal{M}_0$.
...
\medskip
\noindent\textbf{Part III (Symmetry-breaking criterion).} Let $G$ be a Lie group acting smoothly on $\mathcal{P}$, and let $\mathrm{Fix}(G) = \{p \in \mathcal{P} : g \cdot p = p \text{ for all } g \in G\}$ be the fixed-point set. Let $\beta_0 \in \mathcal{B}$ achieve the maximal rank $r(0) = \mathrm{rank}\, dF_0(\beta_0)$.

\textbf{Key point.} We work on the quotient $\mathcal{P}/G$ (equivalently, gauge-fix by choosing orbit representatives). Orbit directions are \textit{not} counted toward rank---they are quotiented out. The rank increase comes from \textbf{$G$-invariant observables} (e.g., order parameter magnitude $r$) becoming parameter-sensitive when symmetry breaks.
...
\begin{enumerate}
    ...
    \item[(S3)] \textbf{Invariant-observable activation}: There exists $v \in \ker(dF_0(\beta_0))$ and a $G$-invariant observable $\phi: \mathcal{P} \to \mathbb{R}$ such that $d(\phi \circ F_\kappa)|_{\beta_0}(v) \neq 0$ for $\kappa > \kappa_c$
\end{enumerate}
Then for $\kappa > \kappa_c$:
\begin{equation}
r(\kappa) \geq r(0) + 1
...
\caption{\textbf{Eigenvalue emergence under coupling.} Fisher eigenvalues on the Gaussian manifold $\mathcal{P}$ (coordinates $(v, \rho)$), showing how the accessible family $\mathcal{M}_{\text{acc}}(\kappa)$ gains dimension. (A) Coupled OU: the correlation direction (third eigenvalue) emerges immediately at $\kappa > 0$. (B) Kuramoto: the order-parameter-sensitivity eigenvalue emerges at $K_c$. Both demonstrate coupling-induced identifiability activation (Theorem~\ref{thm:main}).}
...
\textbf{Generic vs symmetric.} For generic $(\gamma_1, \sigma_1, \gamma_2, \sigma_2)$, the map $\beta \mapsto (\Sigma_{11}, \Sigma_{22}, \Sigma_{12})$ has rank 3 for $\kappa > 0$ and rank 2 for $\kappa = 0$. We present the symmetric case $\gamma_1 = \gamma_2 = \gamma$, $\sigma_1 = \sigma_2 = \sigma$ for intuition; the rank transition persists in the general case.
